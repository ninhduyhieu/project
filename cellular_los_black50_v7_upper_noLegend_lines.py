...
class CellularNetworkReceivedPower:
    def __init__(self, num_ues=5, manhattan_ratio: float = 0.7,
        ...
        self.steps, self.time_per_step = (60 if fast_mode else 100), 3
        ...
        for i in range(self.num_ues):
            for key in ['x','y','huong','BS_ketnoi','prx_hientai','SINR','PacketLoss','speed']:
    ...
    def setup_plot(self):
        ...
        self.ax.set_title(f'LOS nếu đi qua MÀU ĐEN > 50% | origin=UPPER | Khu vực: {int(self.width)} x {int(self.height)} m')
        plt.draw()
    ...
    def save_data_to_csv(self):
        ...
        for i in range(self.num_ues):
            cols += [f'ue{i}_x', f'ue{i}_y', f'ue{i}_huong', f'ue{i}_BS_ketnoi',
                     f'ue{i}_prx_hientai', f'ue{i}_speed']
